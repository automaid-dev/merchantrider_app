...
/// Editable rider profile — matches RiderProfileController::profileUpdate's
/// field list exactly. Pre-fills from the values already loaded on the
/// read-only profile screen (passed in via [initialUser]) so this doesn't
/// need a second fetch.
class RiderProfileEditScreen extends ConsumerStatefulWidget {
  const RiderProfileEditScreen({super.key, required this.initialUser});
  final Map<String, dynamic> initialUser;

  @override
  ConsumerState<RiderProfileEditScreen> createState() => _RiderProfileEditScreenState();
}

class _RiderProfileEditScreenState extends ConsumerState<RiderProfileEditScreen> {
  ...
  void initState() {
    ...
    final rider = u['rider'] as Map<String, dynamic>?;
    ...
    _addressLine1 = TextEditingController(text: u['address_line_1']?.toString() ?? '');
    _addressLine2 = TextEditingController(text: u['address_line_2']?.toString() ?? '');
    _unitNo = TextEditingController(text: u['unit_no']?.toString() ?? '');
    _block = TextEditingController(text: u['block']?.toString() ?? '');
    _postcode = TextEditingController(text: u['postcode']?.toString() ?? '');
    ...
    _emergencyName = TextEditingController(text: rider?['emergency_name']?.toString() ?? '');
    _emergencyPhone = TextEditingController(text: rider?['emergency_phone']?.toString() ?? '');
    _emergencyRelation =
        TextEditingController(text: rider?['emergency_relation']?.toString() ?? '');
    _bankName = rider?['bank_name']?.toString();
    _bankNo = TextEditingController(text: rider?['bank_no']?.toString() ?? '');
  }
  ...
  void dispose() {
    for (final c in [
      ...
      _country, _emergencyName, _emergencyPhone, _emergencyRelation, _bankNo,
    ]) {
      c.dispose();
    }
    super.dispose();
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('EMERGENCY CONTACT', style: TextStyle(color: Colors.grey, fontSize: 12)),
            TextFormField(
