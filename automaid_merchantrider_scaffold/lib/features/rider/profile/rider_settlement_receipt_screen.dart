...
/// /rider/settlement/detail (Api/Rider/SettlementController::detail).
...
class RiderSettlementReceiptScreen extends ConsumerWidget {
  const RiderSettlementReceiptScreen({super.key, required this.hashslug});
  final String hashslug;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final settlementAsync = ref.watch(riderSettlementDetailProvider(hashslug));
    final settingAsync = ref.watch(riderSettingProvider);
  ...
  Future<void> _downloadReceipt(BuildContext context, WidgetRef ref, Map<String, dynamic> settlement) async {
    final setting = await ref.read(riderRepositoryProvider).setting();
    ...
    final bytes = await buildReceiptPdf(
      ...
      rows: rows,
      footerNote: 'Thank you for your work with Automaid.',
...
/// on-screen receipt — same info + same fallback (render nothing if
/// none of it's set yet) as the customer app's booking receipt screen.
