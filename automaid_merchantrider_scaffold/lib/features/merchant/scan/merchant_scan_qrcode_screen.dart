...
/// Wraps POST /merchant/scan/qrcode (ScanQrcodeController::scanQrcode).
/// Scanning a bag's QR shows the customer's booking(s) scheduled for
/// today — this is how a merchant confirms they've got the right bag at
/// pickup, before tapping "Confirm pickup" back on the home screen.
class MerchantScanQrcodeScreen extends ConsumerStatefulWidget {
  const MerchantScanQrcodeScreen({super.key});

  @override
  ConsumerState<MerchantScanQrcodeScreen> createState() => _MerchantScanQrcodeScreenState();
}

class _MerchantScanQrcodeScreenState extends ConsumerState<MerchantScanQrcodeScreen> {
  bool _handled = false;
  ...
  Future<void> _onDetected(String code) async {
    ...
    try {
      final bookings =
          await ref.read(merchantRepositoryProvider).scanQrcode(qrcode: code, type: 'scan');
